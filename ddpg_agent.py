...
class DDPGAgent:
    def __init__(self, state_size, action_size, hidden_layers, gamma,
        ...
        self.gamma = gamma
        self.actor = Network(state_size, hidden_layers[0], 
                             hidden_layers[1], action_size, actor=True).to(device)
        self.critic = Network(2 * (state_size + action_size), hidden_layers[2], 
                              hidden_layers[3], 1).to(device)
    ...
    def step_learn(self, experiences, next_actions_list, idx_agent):
        ...
        # next_actions_list: list (length of num_agents) of 2D tensors (batch_size * action_size)
        ...
        next_full_actions = torch.cat(next_actions_list, dim = 1).to(device) # 2D tensor (batch_size * (num_agents*action_size))
